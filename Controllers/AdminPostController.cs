...
public class AdminPostController : Controller
{
    ...
    public AdminPostController(IPostService postService, ICategoryService categoryService, IImageService imageService, IWebHostEnvironment env)
    {
    ...
    public async Task<IActionResult> Create([Bind("Title,Content,CategoryId,ThumbnailUrl,Summary,Excerpt,IsPublished,IsActive,PublishDate")] Post post, IFormFile thumbnailFile)
    {
        ...
        {
            // If there's a file uploaded, upload it to Cloudinary and set ThumbnailUrl
            if (thumbnailFile != null && thumbnailFile.Length > 0)
            {
                var url = await _imageService.UploadImageAsync(thumbnailFile);
    ...
    public async Task<IActionResult> Edit(int id, [Bind("Id,Title,Content,CategoryId,ThumbnailUrl,Summary,Excerpt,IsPublished,IsActive,PublishDate")] Post post, IFormFile thumbnailFile)
    {
        ...
        if (ModelState.IsValid)
        {
            // If there's a new file uploaded, upload it to Cloudinary and update ThumbnailUrl
            if (thumbnailFile != null && thumbnailFile.Length > 0)
            {
                var url = await _imageService.UploadImageAsync(thumbnailFile);
                if (!string.IsNullOrEmpty(url))
